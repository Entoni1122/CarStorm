...
public class FadeObjectBlockingObject : MonoBehaviour
{
    ...
    private void Start()
    {
        _Camera = GetComponent<SphereCarController>()._camera;
    ...
    private IEnumerator CheckForObjects()
    {
        while (true)
        {
            int hits = Physics.RaycastNonAlloc(
                ...
                (Target.transform.position + TargetPositionOffset - _Camera.transform.position).normalized,
                Hits,
                Vector3.Distance(_Camera.transform.position, Target.transform.position + TargetPositionOffset),
                LayerMask
            );
